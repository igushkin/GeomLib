...
/// Позволяет вычислять площадь многоугольников используя алгоритма Гаусса (Shoelace formula).
...
public class SimplePolygon : IShape
{
    ...
    /// <summary>
    /// Вычисляет площадь многоугольника используя формулу площади Гаусса - https://en.wikipedia.org/wiki/Shoelace_formula.
    ...
    /// При необходимости проверять большое кол-во отрезков можно использовать алгоритм Бентли-Оттманна, которые работает за (O(n * log(n))).
    /// </summary>
    /// <param name="points">Массив вершин</param>
    ...
    /// <summary>
    /// Проверяет, пересекаются ли два отрезка, которые заданны координатами концов. 
    ...
    /// <summary>
    /// Вычисляет векторное произведения для трех точек. 
